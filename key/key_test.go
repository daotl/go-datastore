...
func (ks *BytesKeySuite) SubtestBytesKey(bs []byte, c *C) {
	...
	c.Check(NewBytesKey(bs), DeepEquals, NewBytesKey(bs))
	c.Check(NewBytesKey(bs).String(), DeepEquals, NewBytesKey(bs).String())

	c.Check(NewBytesKey(bs).Child(NewBytesKeyFromString("cchildd")).Bytes(), DeepEquals, kchild)

	c.Check(NewBytesKey(bs).Equal(NewBytesKey(bs)), DeepEquals, true)
	c.Check(NewBytesKey(bs).Equal(NewBytesKey(append([]byte("fdsafdsa"), bs...))), DeepEquals, false)
}
...
func (ks *BytesKeySuite) TestBytesKeyAncestry(c *C) {
	...
	CheckTrue(c, !k2.IsAncestorOf(k2))
	CheckTrue(c, !k1.IsAncestorOf(k1))
	c.Check(k1.Child(NewBytesKeyFromString("D")).String(), DeepEquals, k2.String())
}
